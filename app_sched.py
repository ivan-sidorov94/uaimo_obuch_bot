...
async def date_ot():
    ...
    try:
        if date == ''.join(sqlite_db.cur.execute("SELECT OT FROM users WHERE OT = ?", (date,)).fetchone()):
            ...
            users_lastname_list = ', '.join(row[0] async for row in sqlite_db.cur.execute("SELECT user_lastname FROM users WHERE OT = ?", (date,)).fetchall())
...
async def date_pb():
    ...
    try:
        if date == ''.join(sqlite_db.cur.execute("SELECT PB FROM users WHERE PB = ?", (date,)).fetchone()):
            ...
            users_lastname_list = ', '.join(row[0] async for row in sqlite_db.cur.execute("SELECT user_lastname FROM users WHERE PB = ?", (date,)).fetchall())
...
async def date_eb():
    ...
    try:
        if date == ''.join(sqlite_db.cur.execute("SELECT EB FROM users WHERE EB = ?", (date,)).fetchone()):
            ...
            users_lastname_list = ', '.join(row[0] async for row in sqlite_db.cur.execute("SELECT user_lastname FROM users WHERE EB = ?", (date,)).fetchall())
...
async def date_pjb():
    ...
    try:
        if date == ''.join(sqlite_db.cur.execute("SELECT PJB FROM users WHERE PJB = ?", (date,)).fetchone()):
            ...
            users_lastname_list = ', '.join(row[0] async for row in sqlite_db.cur.execute("SELECT user_lastname FROM users WHERE PJB = ?", (date,)).fetchall())
    ...
    try:
        if date == ''.join(sqlite_db.cur.execute("SELECT PJB1 FROM users WHERE PJB1 = ?", (date,)).fetchone()):
            ...
            users_lastname_list = ', '.join(row[0] async for row in sqlite_db.cur.execute("SELECT user_lastname FROM users WHERE PJB1 = ?", (date,)).fetchall())
            text1 = (date + ' У Вас обучение по Пожарной Безопасности')
            text2 = (date + ' Обучение по Пожарной Безопасности у следующих работников:\n'+ users_lastname_list + '. \nПосле ознакомления ставим лайк.')
            async for row in users_id:
                await bot.send_message(row[0], text1)
...
async def date_opmb():
    ...
    try:
        if date == ''.join(sqlite_db.cur.execute("SELECT OPMB FROM users WHERE OPMB = ?", (date,)).fetchone()):
            ...
            users_lastname_list = ', '.join(row[0] async for row in sqlite_db.cur.execute("SELECT user_lastname FROM users WHERE OPMB = ?", (date,)).fetchall())
...
async def date_med_osm():
    ...
    try:
        if date == ''.join(sqlite_db.cur.execute("SELECT MED_OSM FROM users WHERE MED_OSM = ?", (date,)).fetchone()):
            ...
            users_lastname_list = ', '.join(row[0] async for row in sqlite_db.cur.execute("SELECT user_lastname FROM users WHERE MED_OSM = ?", (date,)).fetchall())
